...
from cnn.neural_network1 import CNN
import os
...
ap.add_argument("-o", "--output1_dir", type=str, default="output1")
...
mnist_data = dataset.data.to_numpy().reshape((dataset.data.shape[0], 28, 28, 1))  # Reshape to add the channel dimension
mnist_data = mnist_data.astype('float32') / 255.0  # Normalize to [0, 1]

# Divide data into testing and training sets
train_img, test_img, train_labels, test_labels = train_test_split(
    mnist_data,  # No need to normalize again
...
for num in np.random.choice(np.arange(0, len(test_labels)), size=(5,)):
    ...
    output_path = os.path.join(args["output1_dir"], f"sample_{num}.png")
